...
function selectedInstallModal(button){
    var data = gather_selected();
    if($.isEmptyObject(data)){
        return;
    }
    var name = 'Installing plugins';
...
function check_slected_install(){
    if(!$.isEmptyObject(gather_selected())){
        $("#install-selected").removeClass("disabled");
    }else{
        $("#install-selected").addClass("disabled");
    }
}
...
$(document).ready(function() {
    ...
    }).on('click', '.btn-info input', function(){
       check_slected_install();
    });
    check_slected_install();
});
