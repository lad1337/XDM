...
def preDB(app_path, datadir):
    ...
    xdm.MONGO_PATH = os.path.join(xdm.DATADIR, xdm.MONGO_NAME)
    log('Set MONGO_PATH to %s' % xdm.MONGO_PATH)
    ...
    from actionManager import load_mongo
    load_mongo()
...
def db():
    ...
    actionManager.persist_mongo()
    if not common.WANTED:
        raise Exception(
            'init went wrong the commons where not set to instances of the db obj')
...
def postDB():
    ...
    common.REPOMANAGER = RepoManager(Repo.objects)
    ...
    common.SYSTEM = common.PM.getPluginByIdentifier(
        common.STARTOPTIONS.systemIdentifer, 'Default') # yeah SYSTEM is a plugin, identifier permits to be sure to get the right plugin.
...
def schedule():
    ...
    common.SCHEDULER.addTask(
        tasks.runSearcher, rate, rate, 'searcher')
    ...
    common.SCHEDULER.addTask(
        tasks.updateAllElements, rate, rate / 2, 'Element Updater')


    # queue worker
    common.SCHEDULER.addTask(
        tasks.checkQ, 2, 10, 'queue worker')
    ...
    common.SCHEDULER.addTask(
        common.REPOMANAGER.autoCache, rate, 10, 'repository cache')
...
def _checkDefaults(resave=False):
    ...
    default_statuss = [
        {'setter': 'WANTED', 'name': 'Wanted', 'hidden': False},
    ...
    for status_config in default_statuss:
        try:
            si = Status.objects.get(name=status_config['name'])
        except DoesNotExist:
            si = Status(
                name=status_config['name'],
                hidden=status_config['hidden'])
            si.save()
        setattr(common, status_config["setter"], si)

    for repo_config in default_repos:
        try:
            Repo.objects.get(url=repo_config['url'])
        except DoesNotExist:
            ri = Repo(
                name=repo_config['name'],
                url=repo_config["url"],
                info_url=repo_config["info_url"]
            )
            ri.save()
